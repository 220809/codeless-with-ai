...
public class CodeSaverExecutor {
    ...
    public static File execute(Object codeResult, GenFileTypeEnum genFileType) {
        return switch (genFileType) {
            case SINGLE_HTML -> SINGLE_HTML_CODE_SAVER.save((SingleHtmlCodeResult) codeResult);
            case MULTI_FILE -> MULTI_FILE_CODE_SAVER.save((MultiFileCodeResult) codeResult);
